...
python scripts/train.py --task=H1_fix_arm_ppo --run_name v1 --headless --num_envs 4096

# Evaluating the Trained PPO Policy 'v1'
# This command loads the 'v1' policy for performance assessment in its environment. 
# Additionally, it automatically exports a JIT model, suitable for deployment purposes.
python scripts/play.py --task=H1_fix_arm_ppo --run_name v1
...
python scripts/H1sim2sim.py --load_model /path/to/logs/XBot_ppo/exported/policies/policy_1.pt
...
- **Training Command**: For training the PPO policy, execute:
  ```
  python humanoid/scripts/train.py --task=H1_fix_arm_ppo --load_run log_file_path --name run_name
  ```
- **Running a Trained Policy**: To deploy a trained PPO policy, use:
  ```
  python humanoid/scripts/play.py --task=H1_fix_arm_ppo--load_run log_file_path --name run_name
  ```
...
- **Mujoco-based Sim2Sim Deployment**: Utilize Mujoco for executing simulation-to-simulation (sim2sim) deployments with the command below:
  ```
  python scripts/H1sim2sim.py --load_model /path/to/export/model.pt
  ```
